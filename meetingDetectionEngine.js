...
const MEETING_DETECTION_DISABLED = /^(1|true|yes|on)$/i.test(
  process.env.OPENWHISPR_DISABLE_MEETING_DETECTION || ""
);
...
class MeetingDetectionEngine {
  constructor(
    ...
    this._detectionDisabled = MEETING_DETECTION_DISABLED;

    if (this._detectionDisabled) {
      debugLogger.warn(
        "Meeting detection is disabled by OPENWHISPR_DISABLE_MEETING_DETECTION",
        {},
        "meeting"
      );
    }
  ...
  handleCalendarReminder(event) {
    if (this._detectionDisabled) return;
  ...
  setPreferences(prefs) {
    if (this._detectionDisabled) {
      this.preferences.processDetection = false;
      this.preferences.audioDetection = false;
  ...
  start() {
    if (this._detectionDisabled) {
      debugLogger.info("Meeting detection engine disabled by environment", {}, "meeting");
      this.meetingProcessDetector.stop();
      this.audioActivityDetector.stop();
      return;
    }
